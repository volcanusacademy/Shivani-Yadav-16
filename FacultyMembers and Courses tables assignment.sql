...
select first_name, last_name, acadmic_degree from facultymembers where acadmic_degree in ("phd");
...
select faculty_id, first_name, last_name,max(YEAR(CURDATE()) - YEAR(hire_date)) as Highest_Years_of_Experience 
from facultymembers; 
...
select max(research_interests) from facultymembers;
...
select course_name,first_name,last_name from facultymembers inner join coursess
on facultymembers.faculty_id =coursess.course_id;
...
SELECT c.course_name, f.first_name, f.tenured
FROM coursess c
JOIN facultymembers f ON c.faculty_id = f.faculty_id
WHERE f.tenured > (SELECT tenured FROM facultymembers WHERE first_name = 'John');
select * from facultymembers;
...
select * from coursess where course_name not like '%Advanced';
...
select course_name,description from coursess where description='Science';
...
select * from facultymembers order by hire_date;
...
SELECT start_date, course_name, dayname(start_date) AS DayOfWeek FROM coursess where start_date>curdate();
select * from coursess;
...
-- 83.Find the courses taught by faculty members with more than 10 years of experience.
 select course_name from coursess c inner join facultymembers f on c.course_id = f.faculty_id where years_of_experience > hire_date ;
